...
class Configurator extends Component
{
    ...
    public function render()
    {
        ...
        if ($this->endTubeNo <= $this->startTubeNo) {
            ...
            $this->error =
                "End Tube Diameter must be greater than Start Tube Diameter";
        } else {
    ...
    public function getMastHeights()
    {
        if (
            $this->noOfActiveTubes == null ||
            $this->lengthMTTubes == null ||
            $this->overlapMTTubes == null ||
            $this->headMTTubes == null
        ) {
        ...
        $this->extendedHeight =
            $this->noOfActiveTubes * $this->lengthMTTubes -
            ($this->noOfActiveTubes - 1) * $this->overlapMTTubes +
            $this->topAdapterThk +
            $this->baseAdapterThk;
        $this->nestedHeight =
            $this->lengthMTTubes +
            ($this->noOfActiveTubes - 1) * $this->headMTTubes +
            $this->topAdapterThk +
            $this->baseAdapterThk;
    ...
    public function getElementCoordinates()
    {
        ...
        $this->mastTubes = array_filter($this->allTubes, function ($element) {
            return $element["no"] >= $this->startTubeNo &&
                $element["no"] <= $this->endTubeNo;
        });
        ...
        foreach (array_reverse($this->mastTubes) as $i => $tube) {
            if (
                $tube["no"] >= $this->startTubeNo &&
                $tube["no"] <= $this->endTubeNo
            ) {
                $this->mastTubes[$i]["bottomCenterPointNested"] =
                    $this->baseAdapterThk + $i * $this->headMTTubes;
                $this->mastTubes[$i]["bottomCenterPointExtended"] =
                    $this->baseAdapterThk +
                    $i * $this->lengthMTTubes -
                    $i * $this->overlapMTTubes;
            }
        ...
        $this->allData["topAdapter"]["bottomCenterPointNested"] =
            $this->nestedHeight - $this->topAdapterThk;
        $this->allData["topAdapter"]["bottomCenterPointExtended"] =
            $this->extendedHeight - $this->topAdapterThk;
    }
    ...
    public function prepareAllData()
    {
        ...
        $this->allData["sideAdapterZ"] = $this->calculateSideAdapterZ();
        ...
        $this->allData["qr"] =
            url("/engineering/configurator?qr=") . implode("-", $q);
    }

    public function calculateSideAdapterZ()
    {
        $h1 =
            $this->mastTubes[0]["bottomCenterPointExtended"] +
            $this->mastTubes[0]["length"];
        $h2 = $this->mastTubes[1]["bottomCenterPointExtended"];
    ...
    {
        $this->allTubes[$i]["areaBasic"] =
            ((pow($od, 2) - pow($id, 2)) * pi()) / 4; // mm2
    ...
    {
        $this->allTubes[$i]["mass"] =
            ($this->materialDensity * $this->realTubeData[$i]["area"]) / 1000; // kg/m
    ...
    public function CalculateInertia($od, $id, $i)
    {
        ...
        $this->allTubes[$i]["inertiaBasic"] =
            (pi() / 64) * (pow($od, 4) - pow($id, 4)); // mm4
    ...
    public function CalculateMomentCapability($od, $id, $i)
    {
        ...
        $this->allTubes[$i]["momentBasic"] =
            ($this->yieldStrength * pi() * (pow($od, 4) - pow($id, 4))) /
            (32 * $od * 1000); // Nm
        $this->allTubes[$i]["moment_capacity"] =
            ($this->yieldStrength * $this->realTubeData[$i]["inertia"]) /
            (0.5 * $od * 1000 * $this->factorOfSafety); // Nm
    ...
    public function ProfileCriticalLoad($od, $id, $i)
    {
        ...
        $this->allTubes[$i]["criticalLoad"] =
            (pi() * $this->E * $this->realTubeData[$i]["inertia"]) /
            (pow($this->tubeBucklingLength, 2) * $this->factorOfSafety);
    ...
    public function EI($od, $id, $i)
    {
        ...
        $this->allTubes[$i]["EI"] =
            $this->E * $this->realTubeData[$i]["inertia"]; // Nmm2
    }
    ...
    public function calculateMaxPayload()
    {
        ...
        if ($this->mastType == "MTPR") {
            ...
            return round(
                min(
                    $minCapability,
                    $this->capacity[$this->mastType]["maxPayload"],
                ),
                0,
            );
        }
    ...
    {
        $maxHeight = collect($this->capacity)
            ->pluck("maxExtendedHeight")
            ->max();
    ...
    */
    ...
    function calculateTubeWindLoads()
    {
        ...
        */
        ...
        foreach ($this->mastTubes as $key => $tube) {
            ...
            $maxHeight = max(
                $Ze,
                $this->terrainCategory[$this->activeTerrainCategory]["zmin"],
            );
            ...
            if ($effective_slenderness <= 10) {
                ...
                $end_effect_factor =
                    0.6023079 * pow($effective_slenderness, 0.0657553); // For slenderness less than or equal to 10
            } else {
                $end_effect_factor =
                    0.698573 +
                    0.001977401 * $effective_slenderness +
                    0.00008741341 * pow($effective_slenderness, 2) -
                    0.00000103591 * pow($effective_slenderness, 3); // For slenderness greater than 10
            }
            ...
            $paramsArray[
                "forceCoefficientWoEndEffect"
            ] = $this->calculateForceCoefficientWOEndEffect($Re, $k_b);
            ...
            $forceCoefficient =
                $paramsArray["forceCoefficientWoEndEffect"] *
                $end_effect_factor; // Force Coefficient (dimensionless)
    ...
    function calculateReferenceArea()
    {
        foreach ($this->mastTubes as $i => $tube) {
            if ($i == 0) {
                ...
                $this->mastTubes[$i]["windLoadActingZ"] =
                    $this->baseAdapterThk + $tube["length"] / 2;
            } else {
                $refArea =
                    $tube["od"] * ($tube["length"] - $this->overlapMTTubes);
                $this->mastTubes[$i]["windLoadActingZ"] =
                    $tube["bottomCenterPointExtended"] +
                    ($tube["length"] + $this->overlapMTTubes) / 2;
            }
    ...
    {
        $coefficent =
            1.2 + (0.18 * log10(10 * $k_b)) / (1 + 0.4 * log10($Re / 1e6));
    ...
    function CalculateDeflection()
    {
        ...
        $moment_at_top_for_z =
            ($this->windLoadOnPayload *
                ($this->extendedHeight + $this->zOffset)) /
            1000; // Nm
    ...
    function areaMomentMethod()
    {
        ...
        foreach ($this->mastTubes as $key => $tube) {
            // Find Root Moment
            $root_moment =
                ($tube["windForce"] * $tube["windLoadActingZ"]) / 1000; //Nm

            $slope = $root_moment / $tube["windLoadActingZ"];

            foreach ($this->mastTubes as $k2 => $t2) {
                $tube_top_end_z =
                    $t2["bottomCenterPointExtended"] + $t2["length"];

                $x_top = $t2["bottomCenterPointExtended"] + $t2["length"];

                $m_bottom = $root_moment - $slope * $tube_bottom_end_z;
                $m_top = $root_moment - $slope * $tube_top_end_z;

                $m_side_adapter =
                    $root_moment - $slope * $this->allData["sideAdapterZ"];

                if ($k2 === 0) {
                    $deflectionArray[$tube["no"]][0] = $root_moment;
                }

                $z_height = "moment_at_" . $tube_top_end_z;
                $z_height_side_adapter =
                    "moment_at_" . $this->allData["sideAdapterZ"];

                $deflectionArray[$tube["no"]][$tube_top_end_z] =
                    $m_top > 0 ? $m_top : 0;
                $deflectionArray[$tube["no"]][$this->allData["sideAdapterZ"]] =
                    $m_side_adapter > 0 ? $m_side_adapter : 0;

                //$deflectionArray[$tube["no"]]["ei"][$this->allData['sideAdapterZ']] = $m_side_adapter > 0 ? $m_side_adapter : 0;
                //$deflectionArray[$key]["ei_at_".$tube_top_end_z."_bottom".$k2] = $t2["EI"];

                $tube_bottom_end_z = $tube_top_end_z;
            }
        }

        $totalMoments = [];

        foreach ($deflectionArray as $k => $values) {
            foreach ($values as $z => $moment) {
                if (isset($totalMoments[$z])) {
                    $totalMoments[$z] += $moment;
                } else {
                    $totalMoments[$z] = $moment;
                }
            }
        ...
        foreach ($this->mastTubes as $key => $tube) {
            $z_top = $tube["bottomCenterPointExtended"] + $tube["length"];

            $this->mastTubes[$key]["moments"][$z_bottom] =
                $totalMoments[$z_bottom];
            $this->mastTubes[$key]["moments"][$z_top] =
                $totalMoments[$z_top];
            $this->mastTubes[$key]["moments"]["ei"] = $tube["EI"];

            $z_bottom = $z_top;
        }

        $this->deflectionWithAreaMomentMethod(1500,$totalMoments);

        //dd($deflectionArray, $totalMoments, $this->mastTubes);
    }
    ...
    function deflectionWithAreaMomentMethod($height,$totalMoments) {

        // Sort totalMoments per z
        ksort($totalMoments);
        ...
        $pairCounter = 0;
        $firstZ = 0;
        $previousValue = $totalMoments[0];

        $momentPairs[$counter]["moments"][$firstZ] = $previousValue;

        foreach ($totalMoments as $z => $moment) {

            if ($z > 0) {
                $momentPairs[$counter]["moments"][$firstZ] = $previousValue;
                $momentPairs[$counter]["moments"][$z] = $moment;
                $momentPairs[$counter]["EI"] = 3446;
                $counter++;
            } else {
                $momentPairs[$counter]["EI"] = 3446;
                $momentPairs[$counter]["moments"][$z] = $moment;
        ...
        foreach ($momentPairs as $k => $pair) {
            $mb = false;

            foreach ($pair["moments"] as $z_value => $moment_value) {
                if (!$mb) {
                    $mb = (float) $moment_value;
                    $zb = (float) $z_value;
                } else {
                    $mt = (float) $moment_value;
                    $zt = (float) $z_value;
                }
            }

            $sonuclar = $this->findAreaMoment($zb,$zt,$mb,$mt);

            $momentPairs[$k]["xbar"] = $sonuclar["xbar"];
            $momentPairs[$k]["moment_area"] = $sonuclar["moment_area"];
        }

        dd($totalMoments,$momentPairs);
    ...
    function findAreaMoment($zb,$zt,$mb,$mt) {

    //dd($zb,$zt,$mb,$mt);
        $dz = abs($zt-$zb)/1000;
        $dm = abs($mt-$mb);

        $xbar = $dz*(3*$mt+2*$dm)/(3*(2*$mt+$dm)); // with respect to right side
    ...
    public function calculateMTPRMass()
    {
        ...
        $this->mastWeightBreakdown["baseFlange"] =
            0.755 * ($this->endTubeNo - 4) + 3.3; // kg
        ...
        foreach ($this->mastTubes as $key => $tube) {
            ...
            $this->mastWeightBreakdown["fixedTubeHeadFlanges"][
                $tube["no"]
            ] = $fixedFlangeWeight; // kg
        ...
        foreach ($this->mastTubes as $key => $tube) {
            if ($tube["no"] != $this->endTubeNo) {
                ...
                $this->mastWeightBreakdown["ringHolderFlanges"][
                    $tube["no"]
                ] = $ringHolderFlangeWeight; // kg
        ...
        foreach ($this->mastTubes as $key => $tube) {
            if ($tube["no"] != $this->endTubeNo) {
                ...
                $this->mastWeightBreakdown["iceBreakers"][
                    $tube["no"]
                ] = $iceBreakerWeight; // kg
        ...
        // Payload Adapter
        $this->mastWeightBreakdown["payloadAdapter"] =
            0.3 * ($this->startTubeNo - 9) + 2.9; // kg
        $this->mastWeight +=
            $this->mastWeightBreakdown["payloadAdapter"] * 1.05; // kg with 5% extra for bolts and nuts
    ...
    public function calculateMTWRMass()
    {
        ...
        foreach ($this->mastTubes as $key => $tube) {
            if ($tube["no"] != $this->endTubeNo) {
                $topRollerHolderFlangeWeight =
                    0.05556 * ($tube["no"] - 6) + 0.75; // kg
                $this->mastWeightBreakdown["topRollerHolderFlanges"][
                    $tube["no"]
                ] = $topRollerHolderFlangeWeight; // kg
        ...
        $this->mastWeightBreakdown["baseAdapter"] =
            0.32225 * ($this->endTubeNo - 6) + 3.5; // kg
        $this->mastWeight += $this->mastWeightBreakdown["baseAdapter"]; // kg;

        // Bottom Roller Holder Flanges
        foreach ($this->mastTubes as $key => $tube) {
            if ($tube["no"] != $this->endTubeNo) {
                $bottomRollerHolderFlangeWeight =
                    0.08889 * ($tube["no"] - 6) + 0.4; // kg
                $this->mastWeightBreakdown["bottomRollerHolderFlanges"][
                    $tube["no"]
                ] = $bottomRollerHolderFlangeWeight; // kg
        ...
        $this->mastWeightBreakdown["fixedRollerHolderParts"] =
            0.04 * $noOfFixedRollerHolderParts; // 0.04 kg/roller
        $this->mastWeight +=
            $this->mastWeightBreakdown["fixedRollerHolderParts"]; // kg;

        // Payload Adapter
        $this->mastWeightBreakdown["payloadAdapter"] =
            0.3567 * ($this->startTubeNo - 6) + 1.74; // kg
        $this->mastWeight +=
            $this->mastWeightBreakdown["payloadAdapter"] * 1.05; // kg with 5% extra for bolts and nuts

        // Steel Wire Weight
        $wireLength =
            (2.05 * count($this->mastTubes) * $this->lengthMTTubes) / 1000; // meters
    ...
    public function calculateMTNXMass()
    {
        ...
        foreach ($this->mastTubes as $tube) {
            ...
            if (
                $this->mastTubes[count($this->mastTubes) - 1]["no"] ===
                $tube["no"]
            ) {
                $is_top_tube = true;
            } else {
            ...
            if ($is_bottom_tube) {
                $base_structure_weight =
                    $mtnx_weight_data["bottom_structure"]["C" . $tube["no"]][
                        "with_accessories"
                    ]["weight"];
                $weight_breakdown["base_structure"][
                    "C" . $tube["no"]
                ] = $base_structure_weight;
            ...
            $tube_weight =
                ($mtnx_weight_data["sections"]["tubes"]["C" . $tube["no"]][
                    "weight_m"
                ] *
                    $this->allData["tubeLength"]) /
                1000; // kg
            ...
            $fixed_flange_weight =
                $mtnx_weight_data["sections"]["fixed_flange"][
                    "C" . $tube["no"]
                ]["average"]["weight"];
            $weight_breakdown["fixed_flange"][
                "C" . $tube["no"]
            ] = $fixed_flange_weight;
            ...
            if (!$is_top_tube) {
                $ice_breaker_weight =
                    $mtnx_weight_data["sections"]["ice_breaker"][
                        "C" . $tube["no"]
                    ]["weight"];
                $weight_breakdown["ice_breaker"][
                    "C" . $tube["no"]
                ] = $ice_breaker_weight;
            ...
            if (!$is_bottom_tube) {
                $weight_breakdown["power_screw_frame"]["C" . $tube["no"]] =
                    $mtnx_weight_data["sections"]["power_screw_frame"][
                        "C" . $tube["no"]
                    ]["assy"]["weight"];
                $totalMass +=
                    $weight_breakdown["power_screw_frame"]["C" . $tube["no"]];
            ...
            if (!$is_bottom_tube) {
                $numberOfLowerKeyGuides =
                    $mtnx_weight_data["key_numbers"]["C" . $tube["no"]];
                $weight_breakdown["lower_key_guides"]["C" . $tube["no"]] =
                    $mtnx_weight_data["sections"]["lower_key_guides"][
                        "C" . $tube["no"]
                    ]["weight"] * $numberOfLowerKeyGuides;
                $totalMass +=
                    $weight_breakdown["lower_key_guides"]["C" . $tube["no"]];
            }
            ...
            if (!$is_top_tube) {
                $numberOfUpperKeyGuides =
                    $mtnx_weight_data["key_numbers"]["C" . $tube["no"]];
                $upper_keys_weight =
                    $mtnx_weight_data["sections"]["upper_key_guides"][
                        "C" . $tube["no"]
                    ]["weight"] * $numberOfUpperKeyGuides;
                $weight_breakdown["upper_key_guides"][
                    "C" . $tube["no"]
                ] = $upper_keys_weight;
            ...
            if ($is_top_tube) {
                $weight_breakdown["euler_fixer"]["C" . $tube["no"]] =
                    $mtnx_weight_data["sections"]["euler_fixer"][
                        "C" . $tube["no"]
                    ]["weight"];
                $totalMass +=
                    $weight_breakdown["euler_fixer"]["C" . $tube["no"]];
            ...
            if ($is_top_tube) {
                $weight_breakdown["payload_interface"]["C" . $tube["no"]] =
                    $mtnx_weight_data["sections"]["payload_interface"][
                        "C" . $tube["no"]
                    ]["weight"];
                $totalMass +=
                    $weight_breakdown["payload_interface"]["C" . $tube["no"]];
            ...
            if (!$is_bottom_tube) {
                $weight_breakdown["lock_stopper_on_tubes"]["C" . $tube["no"]] =
                    $mtnx_weight_data["sections"]["lock_stopper_on_tubes"][
                        "C" . $tube["no"]
                    ]["weight"] * 2;
                $totalMass +=
                    $weight_breakdown["lock_stopper_on_tubes"][
                        "C" . $tube["no"]
                    ];
            }
            ...
            if (!$is_top_tube) {
                $weight_breakdown["lock_key"]["C" . $tube["no"]] =
                    $mtnx_weight_data["sections"]["lock_key"][
                        "C" . $tube["no"]
                    ]["weight"] * 2; // 2 lock keys per tube
                $totalMass += $weight_breakdown["lock_key"]["C" . $tube["no"]];
            }

            // Lock Mechanism Mass Calculation
            if (!$is_top_tube && !$is_bottom_tube) {
                $weight_breakdown["lock_mechanism"]["C" . $tube["no"]] =
                    $mtnx_weight_data["sections"]["lock_mechanism"][
                        "C" . $tube["no"]
                    ]["weight"] * 2; // 2 lock mechanisms per tube
                $totalMass +=
                    $weight_breakdown["lock_mechanism"]["C" . $tube["no"]];
            }
        ...
        $power_screw_volume =
            ((pi() *
                pow($mtnx_weight_data["power_screw"]["diameter"] / 1000, 2)) /
                4) *
            $power_screw_length; // Volume of the power screw in m3 (assuming diameter of 50mm)
        $power_screw_mass =
            $power_screw_volume * $mtnx_weight_data["power_screw"]["density"]; // Mass of the power screw in kg (assuming density of steel is 7850 kg/m3)
