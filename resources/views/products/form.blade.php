<div>
    ...
    @csrf
                ...
                @if ( $ecns->count() > 0)
                    ...
                    @foreach ($ecns as $ecn)
                        ...
                        <label class="checkbox is-block">
                            <input type="checkbox" name="ecn{{$ecn->id}}" value="{{$ecn->id}}"
                            @checked(in_array($ecn->id,$ecns))> ECN-{{ $ecn->id }}
                        </label>

                    @endforeach

                @else
                    <p>AAAA{{ $constants['permissions']['list']['noitem'] }}</p>
                @endif
        ...
        <livewire:ck-editor
            edId="ed10"
            wire:model="remarks"
            label='Part/Product Description'
